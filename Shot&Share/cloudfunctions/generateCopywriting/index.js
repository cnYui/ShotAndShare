...
const STYLE_PROMPTS = {
  literary: {
    name: '文艺治愈',
    prompt: '请为这些图片生成三条不同的文艺治愈朋友圈文案，要求：1）语言优美，富有诗意；2）传达温暖正能量；3）适合发朋友圈；4）每条字数控制在50-100字；5）可以适当使用emoji表情；6）三条文案要有不同的表达角度和情感层次。请直接返回三条文案，每条文案用换行符分隔，不要包含任何JSON格式或其他格式说明。'
  },
  humorous: {
    name: '幽默搞笑',
    prompt: '请为这些图片生成三条不同的幽默搞笑朋友圈文案，要求：1）语言轻松幽默；2）有趣好玩，能让人会心一笑；3）适合发朋友圈；4）每条字数控制在30-80字；5）可以适当使用emoji表情和网络流行语；6）三条文案要有不同的幽默点和表达方式。请直接返回三条文案，每条文案用换行符分隔，不要包含任何JSON格式或其他格式说明。'
  },
  inspirational: {
    name: '励志正能量',
    prompt: '请为这些图片生成三条不同的励志正能量朋友圈文案，要求：1）积极向上，充满正能量；2）能够激励和鼓舞人心；3）适合发朋友圈；4）每条字数控制在40-100字；5）可以适当使用emoji表情；6）三条文案要有不同的励志角度和激励方式。请直接返回三条文案，每条文案用换行符分隔，不要包含任何JSON格式或其他格式说明。'
  },
  romantic: {
    name: '浪漫温馨',
    prompt: '请为这些图片生成三条不同的浪漫温馨朋友圈文案，要求：1）语言温柔浪漫；2）营造温馨美好的氛围；3）适合发朋友圈；4）每条字数控制在40-90字；5）可以适当使用emoji表情；6）三条文案要有不同的浪漫情调和温馨表达。请直接返回三条文案，每条文案用换行符分隔，不要包含任何JSON格式或其他格式说明。'
  },
  philosophical: {
    name: '哲理深度',
    prompt: '请为这些图片生成三条不同的富有哲理的朋友圈文案，要求：1）有深度和思考性；2）能引发共鸣和思考；3）适合发朋友圈；4）每条字数控制在50-120字；5）可以适当使用emoji表情；6）三条文案要有不同的哲理角度和思考维度。请直接返回三条文案，每条文案用换行符分隔，不要包含任何JSON格式或其他格式说明。'
  },
  daily: {
    name: '日常生活',
    prompt: '请为这些图片生成三条不同的日常生活朋友圈文案，要求：1）贴近生活，真实自然；2）语言轻松随意；3）适合发朋友圈；4）每条字数控制在30-80字；5）可以适当使用emoji表情；6）三条文案要有不同的生活感悟和表达方式。请直接返回三条文案，每条文案用换行符分隔，不要包含任何JSON格式或其他格式说明。'
  },
  travel: {
    name: '旅行记录',
    prompt: '请为这些图片生成三条不同的旅行记录朋友圈文案，要求：1）记录旅途美好，分享见闻；2）语言生动有趣；3）适合发朋友圈；4）每条字数控制在40-100字；5）可以适当使用emoji表情；6）三条文案要有不同的旅行感受和记录角度。请直接返回三条文案，每条文案用换行符分隔，不要包含任何JSON格式或其他格式说明。'
  },
  food: {
    name: '美食分享',
    prompt: '请为这些图片生成三条不同的美食分享朋友圈文案，要求：1）诱人美食，味蕾享受；2）语言生动诱人；3）适合发朋友圈；4）每条字数控制在30-80字；5）可以适当使用emoji表情；6）三条文案要有不同的美食描述和感受表达。请直接返回三条文案，每条文案用换行符分隔，不要包含任何JSON格式或其他格式说明。'
  },
  fashion: {
    name: '时尚潮流',
    prompt: '请为这些图片生成三条不同的时尚潮流朋友圈文案，要求：1）展现个性，引领潮流；2）语言时尚有范；3）适合发朋友圈；4）每条字数控制在30-80字；5）可以适当使用emoji表情；6）三条文案要有不同的时尚态度和个性表达。请直接返回三条文案，每条文案用换行符分隔，不要包含任何JSON格式或其他格式说明。'
  },
  fitness: {
    name: '运动健身',
    prompt: '请为这些图片生成三条不同的运动健身朋友圈文案，要求：1）健康生活，活力满满；2）语言积极向上；3）适合发朋友圈；4）每条字数控制在30-80字；5）可以适当使用emoji表情；6）三条文案要有不同的运动感受和健身态度。请直接返回三条文案，每条文案用换行符分隔，不要包含任何JSON格式或其他格式说明。'
  },
  work: {
    name: '职场励志',
    prompt: '请为这些图片生成三条不同的职场励志朋友圈文案，要求：1）工作感悟，职场正能量；2）语言专业励志；3）适合发朋友圈；4）每条字数控制在40-100字；5）可以适当使用emoji表情；6）三条文案要有不同的职场感悟和励志角度。请直接返回三条文案，每条文案用换行符分隔，不要包含任何JSON格式或其他格式说明。'
  },
  friendship: {
    name: '友情岁月',
    prompt: '请为这些图片生成三条不同的友情岁月朋友圈文案，要求：1）珍贵友谊，温暖陪伴；2）语言真挚感人；3）适合发朋友圈；4）每条字数控制在40-90字；5）可以适当使用emoji表情；6）三条文案要有不同的友情表达和情感层次。请直接返回三条文案，每条文案用换行符分隔，不要包含任何JSON格式或其他格式说明。'
  },
  family: {
    name: '家庭温馨',
    prompt: '请为这些图片生成三条不同的家庭温馨朋友圈文案，要求：1）家的温暖，亲情无价；2）语言温馨感人；3）适合发朋友圈；4）每条字数控制在40-90字；5）可以适当使用emoji表情；6）三条文案要有不同的家庭情感和温馨表达。请直接返回三条文案，每条文案用换行符分隔，不要包含任何JSON格式或其他格式说明。'
  },
  nature: {
    name: '自然风光',
    prompt: '请为这些图片生成三条不同的自然风光朋友圈文案，要求：1）大自然之美，心灵净化；2）语言优美诗意；3）适合发朋友圈；4）每条字数控制在40-100字；5）可以适当使用emoji表情；6）三条文案要有不同的自然感受和美景描述。请直接返回三条文案，每条文案用换行符分隔，不要包含任何JSON格式或其他格式说明。'
  }
}
...
exports.main = async (event, context) => {
  ...
  try {
    ...
    try {
      ...
      console.log('JSON解析成功，解析结果:', JSON.stringify(parsed, null, 2))
      if (parsed.copywritings && Array.isArray(parsed.copywritings)) {
        ...
        console.log('从JSON格式解析的文案数组:', copywritings)
      } else {
        console.warn('JSON格式不正确，尝试按换行符分割')
        throw new Error('JSON格式不正确')
      }
    } catch (parseError) {
      console.log('不是JSON格式，按换行符分割文案内容')
      // 按换行符分割文案，过滤空行和无效内容
      copywritings = generatedContent
        .split('\n')
        .map(line => line.trim())
        .filter(line => line && line.length > 0)
        .filter(line => !line.includes('json') && !line.includes('JSON') && !line.includes('格式'))
      console.log('按换行符分割的文案数组:', copywritings)
    }
    
    // 确保至少有一条文案
    if (copywritings.length === 0) {
      console.warn('没有解析到任何文案，使用原始内容作为单条文案')
      copywritings = [generatedContent.trim()]
    }
    
    console.log('最终文案数量:', copywritings.length)
    console.log('最终文案内容:', copywritings)
